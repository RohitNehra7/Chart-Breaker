...
import {
  StockMetadata,
  StockSymbolData,
} from '../interfaces/equityData.interface';
...
export async function fetchStockList(): Promise<StockMetadata[]> {
  ...
  const stocksMetadata: StockMetadata[] = response.data.map(
    (stockData: { metadata: StockMetadata }) => stockData?.metadata
  );

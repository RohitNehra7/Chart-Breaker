...
const TickerItem: React.FC<TickerItemProps> = ({ data }) => {
  ...
  return (
    <div
      className={`${styles['ticker-item']} ${isPositive ? styles['ticker-positive'] : styles['ticker-negative']}`}
    >
      <div className={styles['ticker-box']}>
        ...
        <span
          className={`${styles['percentage-change']} ${isPositive ? styles['positive'] : styles['negative']}`}
        >
          {isPositive ? (
